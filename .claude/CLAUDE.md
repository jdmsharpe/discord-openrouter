...
- Chat uses the official `openrouter` Python SDK. The typed request model `ChatRequestReasoning` declares only
  ...
  can be reinstated for real (last checked against openrouter 1.2.19 on 2026-09-22: still exactly `effort` + `summary`; `chat.send_async`'s 49 parameters are unchanged except that `plugins` accepts one more member, `SwitchyardRouterPlugin`, and the `ChatResult` / `ChatChoice` / `ChatUsage` / `CostDetails` models are unchanged since 1.1.121; the 1.1 → 1.2 minor bump came from a Speakeasy server override on an alpha endpoint the bot does not call). Video and model listing go through raw `httpx` calls wrapped by `_request_with_retries` in `client.py` (exponential backoff + jitter on 429/500/502/503/504, respects `Retry-After` header). Image and STT go through `create_chat_completion` on the typed SDK instead, and TTS streams over raw `httpx` in `_stream_audio_completion` with no retry wrapper.
